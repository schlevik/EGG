...
def main(opts):
    ...
    # this is for WandB hyperparam opt
    opts = wandb.config
    ...
    # the number of features for the Receiver (input) and the Sender (output) is given by
    # n_max (highest summand)* n_summands
    # technically, could be n_max -1
    n_features = opts.n_max * opts.n_summands
    # we reuse the receiver for the recognition game, architecture suits for our purposes.
    ...
    # similar is the case with the sender
    ...
    # the implementation differs slightly depending on whether communication is optimized via Gumbel-Softmax ('gs')
    # or Reinforce ('rf', default)
    if opts.mode.lower() == "gs":
        # ===== This is largely untouched from play.py =======
        # in the following lines, we embed the Sender and Receiver architectures into
        # standard EGG wrappers that are appropriate for Gumbel-Softmax optimization
        # the Sender wrapper takes the hidden layer produced by the core agent architecture
        # we defined above when processing input, and uses it to initialize
        ...
        if opts.max_len == 1:
            ...
            # the Receiver wrapper takes the symbol produced by the Sender at each step
            # (more precisely, in Gumbel-Softmax mode, a function of the overall probability
            # of non-eos symbols upt to the step is used), maps it to a hidden layer through a RNN,
            # and feeds this hidden layer to the
            # core Receiver architecture we defined above
            # (possibly with other Receiver input, as determined by the core architecture) to generate the output
        ...
        # callback functions can be passed to the trainer object (see below) to operate at
        # certain steps of training and validation
        # for example, the TemperatureUpdater (defined in callbacks.py in the core directory)
        # will update the Gumbel-Softmax temperature hyperparameter
        # after each epoch
        ...
        if opts.max_len == 1:
            ...
            # this is for transformer sender
    ...
    if opts.print_validation_events == True:
        ...
        # validation pass after full training and save all results on the eval file
        # in a separate json file: look at PrintValidationEvents in
        ...
        trainer = core.Trainer(
            ...
            callbacks=callbacks
                      + [
                          ...
                          res if opts.ax_hp else core.ConsoleLogger(print_train_loss=True, as_json=True,
                                                                    every_x=opts.log_every),
...
def run_hpopt(train_and_eval_single_step: callable, hp_dict, opts):
    """
    This is run when we perform ax hp optimisation.

    :param train_and_eval_single_step:
        Callable that trains and evaluates a single step and returns the tracked metric.
    :param hp_dict:
        dict of hyper-params to optimise
    :param opts:
        all the other hyper-params not optimised
    :return:
    """
...
if __name__ == "__main__":
    ...
    # if hp dict supplied, run hpopt with ax, otherwise just run main
